...
def setup():
    size(220,100)
    global library, Madzia, Kasztan
    books = ["Naocznosc", "Sens Sztuki", "Harry Potter", "Wstep do kasztanizmu"]
...
def mouseClicked(): 
    if mouseX >100 and mouseX<200:
        if mouseY >10 and mouseY <30:
            library.lendBook(Kasztan.requestBook("Wstep do kasztanizmu"))
            library.lendBook(Madzia.requestBook("Naocznosc"))
            
        if mouseY >40 and mouseY <60:
            library.addBook(Madzia.returnBook())
            library.addBook(Kasztan.returnBook())
